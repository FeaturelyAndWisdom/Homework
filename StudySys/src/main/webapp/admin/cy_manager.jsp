...
<script>
    ...
    function submitinfo() {
        ...
        var jsoninf = {"word":cy,"pinyin":py,"type":1};
        $.ajax({
            type:'post',
    ...
    function getPinyin()
    {
        var value = document.getElementById('inputcy').value;
        var result = '';
        if(value)
        {
...
<table align="center" id="addshow" width="450px" style="display: none">
    ...
    <tr>
        <td align="center" style="width:50%">成语：</td>
        <td align="center" style="width:50%">
            <input type="text" placeholder="请输入要添加的成语" onchange="showpy()" style="width:200px;height: 35px;" id="inputcy">
        </td>
    </tr>
    <tr>
...
<div id="cylst">
<input type="button" value="添加新成语" onclick="additem()" style="width: 150px;height: 30px;float: right;margin-bottom: 20px">
<table border="1" cellspacing="3" align="center" width="750px">
    ...
    <c:forEach items="${requestScope.knowlist}" var="cy">
        <tr>
            <td align="center">${cy.kid}</td>
            <td align="center">${cy.word}</td>
            <td align="center">${cy.pinyin}</td>
            <td align="center"><fmt:formatDate value="${cy.createtime}" pattern="yyyy-MM-dd hh:mm:ss"/></td>
            <td align="center">
                <c:if test="${cy.isdel==0}">
                    <a href="javascript:;" onclick="deleteknow(${cy.kid})">删除</a>
                </c:if>
                <c:if test="${cy.isdel==1}">
                    已删除
                </c:if>
            </td align="center">
        </tr>
